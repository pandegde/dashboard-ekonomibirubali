...
const file = path.join(__dirname, "../data/indikatorKinerjaKunci.json");

// Struktur utama database
type Database = {
  indikatorKinerjaKunci: Record<string, Indikator[]>; // data per tahun
  [key: string]: any; // agar bisa diakses dengan key string
};

const adapter = new JSONFile<Database>(file);
const db = new Low(adapter, { indikatorKinerjaKunci: {} });
...
async function initDB() {
  await db.read();
  db.data ||= { indikatorKinerjaKunci: {} };
}

// -------------------------------------------------------
// 4️⃣ Ambil data berdasarkan key (default: "indikatorKinerjaKunci")
// -------------------------------------------------------
export async function getData(key = "indikatorKinerjaKunci") {
  await initDB();
  return db.data?.[key] || {};
}
...
export async function saveData(year: string, payload: Indikator[]) {
  ...
  if (!db.data!.indikatorKinerjaKunci) db.data!.indikatorKinerjaKunci = {};

  // Menyimpan data untuk tahun tertentu
  db.data!.indikatorKinerjaKunci[year] = payload;
